...
public abstract class ApiBaseController {
	 ...
	 */
	protected boolean beanValidator(ResultBean<List<String>> resultBean, Object object, Class<?>... groups) {
		try{
			BeanValidators.validateWithException(validator, object, groups);
		}catch(ConstraintViolationException ex){
			List<String> list = BeanValidators.extractPropertyAndMessageAsList(ex, ": ");
			list.add(0, "数据验证失败：");
			resultBean.setCode(ResultEnum.ERROR_VALIDATE.getCode());
			resultBean.setMessage(ResultEnum.ERROR_VALIDATE.getMessage());
			resultBean.setBodyData(list);
			return false;
		}
		return true;
	}
