...
public class WebRTCSigApi {
     ...
     * @return
     */
    public static String getPrivateMapKey(int roomid,String userid) {
    	int sdkappid = 1400207995;
    	try {
	        Resource privateKeyResource = resourceLoader.getResource("private_key");
	        Resource publicKeyResource = resourceLoader.getResource("public_key");
	        
	    	byte[] privateKey = new byte[(int)privateKeyResource.getFile().length()];
	    	byte[] publicKey = new byte[(int)publicKeyResource.getFile().length()];
       
            //读取私钥的内容
            //PS:不要把私钥文件暴露到外网直接下载了哦
            FileInputStream in1 = new FileInputStream(privateKeyResource.getFile());
            in1.read(privateKey);
            in1.close();
            //读取公钥的内容
            FileInputStream in2 = new FileInputStream(publicKeyResource.getFile());
            in2.read(publicKey);
            in2.close();
            ...
            //生成userSig
            String userSig = api.genUserSig(userid, 60*60*24*30*50);
            //生成privateMapKey
    ...
    public static String getPrivateMapKey(String userid) {
    	int sdkappid = 1400207995;
    	int roomid = 0;
    	try {
	        Resource privateKeyResource = resourceLoader.getResource("private_key");
    ...
    public static String getPrivateMapKey_book (String userid) {
    	int sdkappid = 1400230157;
    	int roomid = 0;
    	try {
	        Resource privateKeyResource = resourceLoader.getResource("private_key2.txt");
	        Resource publicKeyResource = resourceLoader.getResource("public_key2.txt");
    ...
    public void setPrivateKey(String privateKey) {
        ...
        .replace("\n", "");
        byte[] encodedKey = Base64.decodeBase64(privateKeyPEM.getBytes());
    ...
    public void setPublicKey(String publicKey) {
        ...
        .replace("\n", "");
        byte[] encodedKey = Base64.decodeBase64(publicKeyPEM.getBytes());
    ...
    public String genUserSig(String userid, int expire) {
        ...
        String sig = Base64.encodeBase64String(signBytes);
    ...
    public String genPrivateMapKey(String userid, int roomid, int expire) {
        ...
        String userbuf = Base64.encodeBase64String(bytes);
    ...
    public static void main(String[] args) {
        try {
            String privateMapKey = getPrivateMapKey_book("admin");
        } catch (Exception e ) {
            e.printStackTrace();
        }
    }
