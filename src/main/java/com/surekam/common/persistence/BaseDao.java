...
import com.surekam.common.utils.Reflections;
...
//	        page.setCount(Long.valueOf(createQuery(countQlString, parameter).uniqueResult().toString()));
		...
		if (StringUtils.isNotBlank(page.getOrderBy())){
			ql = removeOrders(ql) + " order by " + page.getOrderBy();
		}
	 ...
	 * @return
	 */
	public <E> Page<E> findUnion(Page<E> page, String qlString, String countHqlString){
    	return findUnion(page, qlString, countHqlString, null);
    }
	 ...
	 */
    @SuppressWarnings("unchecked")
	public <E> Page<E> findUnion(Page<E> page, String qlString, String countHqlString, Parameter parameter){
		// get count
    	if (!page.isDisabled() && !page.isNotCount()){
	        String countQlString = "select count(*) " + countHqlString;  
//	        page.setCount(Long.valueOf(createQuery(countQlString, parameter).uniqueResult().toString()));
	        Query query = createQuery(countQlString, parameter);
	        List<Object> list = query.list();
	        if (list.size() > 0){
	 ...
	 * @return
	 */
	public int updateDelFlagForeignKey(String foreignKey, String delFlag){
		return update("update "+entityClass.getSimpleName()+" set delFlag = :p2 where office_id = :p1", 
				new Parameter(foreignKey, delFlag));
	}
	/**
	 * 删除
	 * @param id
	 * @return
	 */
	public int deleteDataById(Serializable id){
		return update("delete "+entityClass.getSimpleName()+" where id = :p1", 
				new Parameter(id));
	}
	 ...
	 * @return
	 */
	public int deleteByXGXTId(Serializable id){
		return update("update "+entityClass.getSimpleName()+" set states='" + BaseEntity.STATE_FLAG_DEL + "' where id = :p1", 
				new Parameter(id));
	}
	
	public int resumeByXGXTId(Serializable id){
		return update("update "+entityClass.getSimpleName()+" set states='" + BaseEntity.STATE_FLAG_UPDATE+ "' where id = :p1", 
				new Parameter(id));
	}
	 ...
	 * @return
	 */
	public int deleteByModelId(Serializable id){
		return update("update "+entityClass.getSimpleName()+" set states='" + BaseEntity.STATE_FLAG_DEL + "' where modelId = :p1", 
				new Parameter(id));
	}
	 ...
	 * @return
	 */
	public int deleteByXGXTId(Serializable id, String likeParentIds){
		return update("update "+entityClass.getSimpleName()+" set states = '" + BaseEntity.STATE_FLAG_DEL + "' where id = :p1 or parentIds like :p2",
				new Parameter(id, likeParentIds));
	}
	 ...
	 * @return
	 */
	public int updateGXBZ(Serializable id, String delFlag){
		return update("update "+entityClass.getSimpleName()+" set states = :p2 where id = :p1", 
				new Parameter(id, delFlag));
	}
...
//	        page.setCount(Long.valueOf(createSqlQuery(countSqlString, parameter).uniqueResult().toString()));
		...
		if (StringUtils.isNotBlank(page.getOrderBy())){
			sql = removeOrders(sql) + " order by " + page.getOrderBy();
		}
	...
	public int updateBySql(String sqlString){
		return getSession().createSQLQuery(sqlString).executeUpdate();
	}
		...
		if (parameter != null) {
            ...
            for (String string : keySet) {
                ...
                if(value instanceof Collection<?>){
                    ...
                    query.setTimestamp(string,(Date)value);
                }else{
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <E> List<E> findBySql2(String sqlString, Parameter parameter, Class<?> resultClass){
		SQLQuery query = createSqlQuery(sqlString, parameter);
		query.addEntity(resultClass);
		return query.list();
	}
	...
	/**
	 * SQL 查询本周扫码，本月扫码，全部扫码次数，返回Integer
	 * @param sqlString
	 * @param resultClass
	 * @param parameter
	 * @return
	 */
	@SuppressWarnings("unchecked")
	public List<Integer> findBySql3(String sqlString){
		SQLQuery query = createSqlQuery(sqlString, null);
		return query.list();
	}
}