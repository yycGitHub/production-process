...
import com.surekam.common.utils.FreeMarkers;
import com.surekam.generate.common.TableInfoUtil;
...
public class Generate {
	...
	public static void main(String[] args) throws Exception {
		...
		String packageName = "com.surekam.modules.agro";
		
		String moduleName = "auditrecord";			// 模块名，例：sys
		String subModuleName = "";				// 子模块名（可选） 
		String className = "AuditRecord";			// 类名，例：user
		String classAuthor = "liwei";		    // 类作者
		String functionName = "巡检记录表";			// 功能名，例：用户
		String tableName = "t_agro_audit_record";
		...
		// 模板文件路径
		String tplPath = StringUtils.replace(projectPath+"/src/main/java/com/surekam/generate/template", "/", separator);
		...
		// 视图文件路径
		String viewPath = StringUtils.replace(projectPath+"/WebContent/WEB-INF/views", "/", separator);
		...
		model.put("sourceTable",TableInfoUtil.getTableInfo(tableName));	
		...
		// 生成 ViewInformation
		template = cfg.getTemplate("viewInformation.ftl");
		content = FreeMarkers.renderTemplate(template, model);
		filePath = viewPath+separator+StringUtils.substringAfterLast(model.get("packageName").toString(),".")
				+separator+model.get("moduleName")+separator+StringUtils.lowerCase(subModuleName)
				+separator+model.get("className")+"Information.jsp";
		writeFile(content, filePath);
		logger.info("ViewForm: {}", filePath);
